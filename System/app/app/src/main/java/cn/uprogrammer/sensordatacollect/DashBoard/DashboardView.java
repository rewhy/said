...
public class DashboardView extends View {
    ...
    public void setRealTimeDValue(double realTimeValue, boolean animEnable, long duration) {
        mHandler.preValue = mRealTimeValue;
        mAnimEnable = animEnable;
        initSizes();
        if (!mAnimEnable) {
            ...
            this.duration = duration;
            mRealTimeValue = (float) realTimeValue;
            mHandler.endValue = (float) realTimeValue;
            mHandler.deltaValue = Math.abs(mHandler.endValue - mHandler.preValue);
            mHandler.sendEmptyMessage(0);
        }
    }
